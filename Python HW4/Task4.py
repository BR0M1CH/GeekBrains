def reader():  # Функция ввода с проверкой
    global mas
    try:
        # Простая проверка ввода на присутсвие степени в строке
        print("Введите квадратное уравнение без знаков умножения:", end="")
...
def finder_a():  # Функция поиска коэффициента "а"
    ...
    if mas[0] == "x":  # Если первый символ списка - "х", то коэф. "а" равен единице
        ...
        a = str("")
        try:
            # Пробуем преобразовать первый символ строки в инт: если преобразовалось, значит на первом месте - цифра, значит знака минус нет
            int(mas[0])
            oper = 0
        except:  # А если не получилось - значит там знак минус, меняем оператор и удаляем этот минус
            oper = 1
            mas.pop(0)
        # Считываем все символы до "х", добавляем их в строку Б, затем конвертируем её в целое
        for i in range(mas.index("^")-1):
            a += mas[i]
        try:
            if oper == 0:
                a = int(a)
        ...
        # Подчищаем наш список вплоть до оператора перед икс или перед целым числом
        while mas[0] != "+" or mas[0] != "-" or mas[0] != "=":
            mas.pop(0)
            if mas[0] == "+" or mas[0] == "-" or mas[0] == "=":
                break
...
def finder_b():  # Функция поиска коэффициента "b"
    ...
    try:
        ...
        # В условие добавился знак равно - на случай, если нулевой степени в строке нет
        while mas[0] != "+" or mas[0] != "-" or mas[0] != "=":
            mas.pop(0)
            if mas[0] == "+" or mas[0] == "-" or mas[0] == "=":
                break
...
def finder_c():  # Функция поиска коэффициента "с"
    ...
    c = str("")
    try:
        # Если в начале строки стоит знак равно, значит нулевого коэф. нет
        mas[0] != "="
